  ...
  <t-form ref="form" :class="['item-container', `login-${type}`]" :data="formData" :rules="FORM_RULES" label-width="0"
    ...
    <template v-if="type == 'password'">
      ...
      <t-form-item name="password">
        <t-input v-model="formData.inputPassword" size="large" :type="showPsw ? 'text' : 'password'" clearable
...
const FORM_RULES: Record<string, FormRule[]> = {
  ...
  inputPassword: [{ required: true, message: '密码必填', type: 'error' }],
...
const onSubmit = async ({ validateResult }) => {
  if (validateResult === true) {
    try {
      formData.value.password = md5(formData.value.inputPassword.trim())
