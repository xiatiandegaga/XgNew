    ...
    <t-form ref="form" :data="formData" :label-width="80" colon @keyup.enter="fetchData">
      <t-row>
          ...
          <t-row :gutter="[24, 24]">
            <t-col :span="4">
              <t-form-item label="品牌名称" name="brandName">
                <t-input v-model="formData.brandName" class="form-item-content" type="search" placeholder="请输入品牌名称"
        ...
        <t-col :span="2" class="operation-container">
          ...
          <t-button @click="handleClickAdd()" v-auth="'mallBrand:add'"> 新建 </t-button>
    ...
    <div class="table-container">
      <t-table :data="data" :columns="COLUMNS" row-key="id" vertical-align="top" :hover="true" :pagination="pagination"
        ...
        <template #op="slotProps">
          <a class="t-button-link" @click="handleClickUpdate(slotProps)" v-auth="'mallBrand:edit'">编辑</a>
          <t-popconfirm content="确认删除吗" @confirm='handleClickDelete(slotProps)'>
            <a class="t-button-link" v-auth="'mallBrand:delete'">删除</a>
          </t-popconfirm>
        </template>
      </t-table>
    </div>
    <mall-brand-form ref="mallBrandForm" @parentFetchData="fetchData" />
  </div>
</template>
<script setup lang="ts">
import { ref, reactive, onMounted } from 'vue';
import { getPageList, logicDelete } from '@/api/mall/mallBrand';
...
import MallBrandForm from './components/MallBrandForm.vue';
...
const filter = () => {
  ...
  if (formData.brandName)
    filterInfo.push({ op: 5, propertyName: 'brandName', value: formData.brandName });
  return filterInfo
}
// 获取dialogForm元素节点
const mallBrandForm = ref<any>(null);;

// 新建Modal
const handleClickAdd = () => {
  mallBrandForm.value.add()
};
// 编辑Modal
const handleClickUpdate = ({ row }) => {
  mallBrandForm.value.update(row.id)
};
