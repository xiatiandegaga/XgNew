...
namespace Cloud.Extensions
{
    public static class StringExtension
    {
        ...
        {
            CheckUtility.NotNull(str, nameof(str));
        ...
        public static T ToEnum<T>(this string value)
            where T : struct
        {
            CheckUtility.NotNull(value, nameof(value));
        ...
        {
            CheckUtility.NotNull(str, nameof(str));
            CheckUtility.NotNull(encoding, nameof(encoding));
        ...
        /// 字符串替换
        /// </summary>
        /// <param name="str">要替换的数据源</param>
        /// <param name="start">替换起始位置（从1开始）</param>
        /// <param name="len">长度</param>
        /// <param name="replace">要替换成的字符串</param>
        /// <returns></returns>
        public static string Insert(this string str, int start, int len, string replace)
        {
            if (start <= 0) start = 1;
            if (str == default || start >= str.Length || len <= 0) return str;
            var strSpan = str.AsSpan();
            if (start - 1 + len >= str.Length)
                return strSpan.Slice(0, start - 1).ToString() + replace;
            else return strSpan.Slice(0, start - 1).ToString() + replace + strSpan.Slice(start - 1 + len).ToString();
        }
    }
}
